...
public class Example1 {
	...
	private static void runExample1() {
		final SparkConf sparkConf = new SparkConf().setAppName("simple test");
		sparkConf.setMaster("local[2]");
		final JavaSparkContext sc = new JavaSparkContext(sparkConf);
		JavaRDD<String> rdd1 = sc.parallelize(Arrays.asList(new String[] { "Hadoop", "Spark", "Hive", "Kudu", "Beeline",
				"Alluxio", "Kylin", "Impala", "Parquet" }));
		JavaRDD<String> rdd2 = sc.parallelize(
				Arrays.asList(new String[] { "Spark", "Presto", "Impala", "Orc", "HDFS", "Tez", "Alluxio", "Kafka" }));
